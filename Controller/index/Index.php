...
class Index extends \Magento\Framework\App\Action\Action
{
    ...
    private function _itemSaveData($attributeModel)
    {
        $attributeId = $attributeModel->load(1)->getData();
        return $attributeId = $attributeId[0]['attribute_id'];
    }
    ...
    private function __filterAdd($params)
    {
            ...
            foreach ($filters as $key => $filter) {
                if (!empty($filter)) {
                    if ($filter['code'] == 'price') {
                        ...
                        $resource       = $ObjectData->get('Magento\Framework\App\ResourceConnection');
                        $connection     = $resource->getConnection();
                        $attributeModel =
                        $ObjectData->get('Magento\Eav\Model\Entity\Attribute')->getCollection()
                            ->addFieldToFilter('attribute_code', $filter['code']);
                        $attributeId = $this->_itemSaveData($attributeModel);
                        if ($attributeModel) {
                            $attributeId = $this->_itemSaveData($attributeModel);
                        } else {
                            continue;
                        ...
                        $this->_filterAddRest($filter, $connection);
